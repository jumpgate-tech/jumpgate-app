...
//
// It is also duplicated — internal/chainlist and internal/capabilities each
// carry their own copy, with THREE different size caps between them (8MB here,
// 64KB in chainlist, 1MB in capabilities). These tests are deliberately written to the same shape
// in all three packages, so whoever finally extracts internal/wsrpc has a
...
// wsFrameCap mirrors the literal this package's reader compares against. It is
// spelled out here rather than imported because the reader inlines it — which
// is itself the reason these three copies have drifted to three different
// limits.
const wsFrameCap = 8 << 20
...
func TestWSReadMessage_RefusesACompleteFrameOverTheCap(t *testing.T) {
	over := bytes.Repeat([]byte("x"), wsFrameCap+1)
	if _, err := wsReadMessage(wsStream(wsFrame{fin: true, opcode: opText, payload: over})); err == nil {
		t.Fatalf("a complete %d-byte frame was accepted (cap is %d)", len(over), wsFrameCap)
	}
	...
	// reported dead.
	atCap := bytes.Repeat([]byte("x"), wsFrameCap)
	got, err := wsReadMessage(wsStream(wsFrame{fin: true, opcode: opText, payload: atCap}))
	if err != nil {
		t.Fatalf("a frame exactly at the %d-byte cap was refused: %v", wsFrameCap, err)
	}
