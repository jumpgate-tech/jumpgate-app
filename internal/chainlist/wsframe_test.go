...
//
// It is also duplicated — internal/capabilities and internal/setup each carry
// their own copy, with THREE different size caps between them (64KB here, 1MB
// there, 8MB in setup). These tests are deliberately written to the same shape
...
func TestWSReadMessage_RefusesACompleteFrameOverTheCap(t *testing.T) {
	over := bytes.Repeat([]byte("x"), maxProbeBytes+1)
	if _, err := wsReadMessage(wsStream(wsFrame{fin: true, opcode: opText, payload: over})); err == nil {
		t.Fatalf("a complete %d-byte frame was accepted (cap is %d)", len(over), maxProbeBytes)
	}
	...
	// reported dead.
	atCap := bytes.Repeat([]byte("x"), maxProbeBytes)
	got, err := wsReadMessage(wsStream(wsFrame{fin: true, opcode: opText, payload: atCap}))
	if err != nil {
		t.Fatalf("a frame exactly at the %d-byte cap was refused: %v", maxProbeBytes, err)
	}
