...
// writeText writes payload as one masked text frame.
//
...
func readMessage(br *bufio.Reader, max int64) ([]byte, error) {
	var msg []byte
	for {
		fin, opcode, payload, err := readFrame(br, max-int64(len(msg)))
		if err != nil {
			return nil, err
		}
		switch opcode {
		...
		case opcodePing, opcodePong: // nothing to do for a request/response exchange
		default:
...
func readFrame(br *bufio.Reader, budget int64) (fin bool, opcode byte, payload []byte, err error) {
	var hdr [2]byte
